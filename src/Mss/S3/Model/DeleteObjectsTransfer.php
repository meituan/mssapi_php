...
namespace Mss\S3\Model;

use Mss\Common\Client\MssClientInterface;
use Mss\Common\Exception\OverflowException;
use Mss\Common\Enum\UaString as Ua;
use Mss\S3\Exception\InvalidArgumentException;
use Mss\S3\Exception\DeleteMultipleObjectsException;
...
{
    /**
     * @var MssClientInterface The Amazon S3 client for doing transfers
     */
    protected $client;
     ...
     *
     * @param MssClientInterface $client Client used to transfer the requests
     ...
     */
    public function __construct(MssClientInterface $client, $bucket, $mfa = null)
    {
        $this->client = $client;
        $this->bucket = $bucket;
        $this->mfa = $mfa;
