...
class S3ClientTest extends \Guzzle\Tests\GuzzleTestCase
{
    ...
    /**
     * @covers Mss\S3\S3Client::isValidBucketName
    ...
    /**
     * @covers Mss\S3\S3Client::factory
     */
    ...
    /**
     * @covers Mss\S3\S3Client::createPresignedUrl
     */
     ...
     */
    public function no_testCanCreateObjectUrls($expires, array $args, $expectedUrl)
    {
        /** @var $client S3Client */
        $client = $this->getServiceBuilder()->get('s3', true);
    ...
    /**
     * @expectedException \Mss\Common\Exception\RuntimeException
     */
    ...
    public function testDeletesMatchingObjects()
    {
        ...
        $this->assertEquals('/foo?delete', $history->getLastRequest()->getResource());
    ...
    public function testUploadsSmallerObjectsUsingPutObject()
    {
        ...
        $this->assertEquals('/test/key', $request->getResource());
    ...
    public function testUploadsLargerObjectsUsingMultipartUploads()
    {
        ...
        $this->assertStringStartsWith('/test/key?uploadId=', $request->getResource());
    ...
    public function testUploadsDirectories()
    {
        ...
        $mockBuild = $this->getMockBuilder('Mss\S3\Sync\UploadSync')
        ...
        $mock = $this->getMockBuilder('Mss\S3\Sync\UploadSyncBuilder')
    ...
    public function testDownloadsBuckets()
    {
        ...
        $mockBuild = $this->getMockBuilder('Mss\S3\Sync\DownloadSync')
            ->disableOriginalConstructor()
            ->setMethods(array('transfer'))
            ->getMock();
        ...
        $mock = $this->getMockBuilder('Mss\S3\Sync\DownloadSyncBuilder')
    ...
    /**
     * @expectedException \Mss\Common\Exception\InvalidArgumentException
     */
    public function testEnsuresSigV4HasRegion()
    {
        $sig = $this->getMockBuilder('Mss\S3\S3SignatureV4')
            ->disableOriginalConstructor()
            ->getMock();
    ...
    {
        $sig = $this->getMockBuilder('Mss\S3\S3Signature')
            ->disableOriginalConstructor()
            ->getMock();
    ...
    public function testCanForceS3SignatureUsingString()
    {
        ...
        $this->assertInstanceOf('Mss\S3\S3Signature', $s3->getSignature());
    }

    public function no_testCanForceSigV4Signature()
    {
        $sig = $this->getMockBuilder('Mss\S3\S3SignatureV4')
            ->disableOriginalConstructor()
            ->getMock();
     ...
     */
    public function no_testEnsuresRegionIsSetWhenUsingV4()
    {
        $s3 = S3Client::factory(array(Options::SIGNATURE => 'v4'));
        $this->assertInstanceOf('Mss\S3\S3SignatureV4', $s3->getSignature());
    }

    public function no_testCanForceS3SignatureV4UsingString()
    {
