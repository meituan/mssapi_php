...
namespace Mss\Tests\S3\Model\MultipartUpload;

use Mss\Common\Enum\DateFormat;
use Mss\S3\Model\MultipartUpload\AbstractTransfer;
use Mss\S3\Model\MultipartUpload\TransferState;
use Mss\S3\Model\MultipartUpload\UploadPart;
use Guzzle\Http\EntityBody;

/**
 * @covers Mss\S3\Model\MultipartUpload\AbstractTransfer
 */
class AbstractTransferTest extends \Guzzle\Tests\GuzzleTestCase
{
    ...
    /** @var \Mss\S3\Model\MultipartUpload\AbstractTransfer */
    protected $transfer;
    ...
    public function prepareTransfer(
        ...
        $uploadId = $this->getMockBuilder('Mss\S3\Model\MultipartUpload\UploadId')
        ...
        if ($useRealClient) {
            ...
            $client = $this->getMockBuilder('Mss\S3\S3Client')
                ->disableOriginalConstructor()
                ->getMock();
        }

        $state = $this->getMockBuilder('Mss\S3\Model\MultipartUpload\TransferState')
            ->disableOriginalConstructor()
            ->getMock();
        ...
        $this->client = $client;
        $this->transfer = $this->getMockForAbstractClass('Mss\S3\Model\MultipartUpload\AbstractTransfer', array(
    ...
    public function partSizeDataProvider()
    {
        return array(
            ...
            array(false, null, null, 'Mss\Common\Exception\RuntimeException')
        );
    }
