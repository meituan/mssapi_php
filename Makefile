...
test:
	/usr/local/bin/phpunit $(TEST)

coverage:
	/usr/local/bin/phpunit --coverage-html=build/artifacts/coverage $(TEST)

integ:
	/usr/local/bin/phpunit -c phpunit.functional.xml $(TEST)
...
burgomaster:
	...
	#curl -s http://raw.githubusercontent.com/mtdowling/Burgomaster/0.0.2/src/Burgomaster.php > build/artifacts/Burgomaster.php
